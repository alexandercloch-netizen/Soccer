...
3. **Authorization is tested as a matrix, not a happy path.** Every write is exercised as visitor, family, coach of this team, coach of another team, and admin. Four of those five must be refused.
...
| Repository contract | Vitest | `tests/contract/` | CI (Postgres service), local with `DATABASE_URL` | The same suite passes against the JSON repo and the Postgres repo: load, save event, save signup, compare-and-set, share-code rotation |
| Request-level | Vitest | `tests/request/` | CI, Netlify | Route handlers and server actions called directly with `Request` objects and a stubbed session: the authorization matrix, zod rejection, rate limits, generic errors, redirect guard, locked gate |
...
- The committed seed (`data/teams/*.json`) is the fixture for read-only tests; write tests use a temp directory (fs repo) or a transaction rolled back after each test (Postgres repo).
...
npm run test:contract # repository contract suite; needs DATABASE_URL for the Postgres half
...
The contract, visual, and AI-eval layers are scaffolded by WP00 in the swarm graph.
